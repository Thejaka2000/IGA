...
with open("bspline_zero_middle.dat", "w") as f:

    for x in np.linspace(xmin, xmax, 700):
...
# Show x-axis only at unique knot values
unique_knots = sorted(set(knots))   # [0,1,2]
tick_string = ", ".join([f'"{v}" {v}' for v in unique_knots])

# GNUplot
caption = "Quadratic B-Spline : ΔΞ = {1,0,1}"
gp = f"""
set terminal qt size 950,600
...
plot for [i=2:{n_basis+1}] 'bspline_zero_middle.dat' using 1:i with lines lw 2 title sprintf("N_%d,2", i-2)
