...
with open("bspline_zero_span.dat", "w") as f:
...
# Show only distinct knot values on x-axis
unique_knots = sorted(set(knots))   # [0,1]
tick_string = ", ".join([f'"{v}" {v}' for v in unique_knots])


# GNUplot
caption = "Quadratic B-Spline : ΔΞ = {0,0,1}"
gp = f"""
...
plot for [i=2:{n_basis+1}] 'bspline_zero_span.dat' using 1:i with lines lw 2 title sprintf("N_%d,2", i-2)
